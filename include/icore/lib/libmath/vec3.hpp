	...
	namespace core {
		namespace libmath {
			...
			public:
				...
				Vec3<T>& operator -- () { --x; --y; --z; return *this; }
				Vec3<T> operator ++ (int) { Vec3<T> tmp = *this; ++* this; return tmp; }
				Vec3<T> operator -- (int) { Vec3<T> tmp = *this; --* this; return tmp; }
				...
				T sumAbs() const { return _function::abs<T>(x) + _function::abs<T>(y) + _function::abs<T>(z); }
				bool emptyAND() const { return x == (T)0 && y == (T)0 && z == (T)0; }
				bool emptyOR() const { return x == (T)0 || y == (T)0 || z == (T)0; }
				std::string string(const char& c = ';') const { return std::to_string(x) + c + std::to_string(y) + c + std::to_string(z); }
				T mini() const { return _function::internalMini<T>(x, _function::internalMini<T>(y, z)); }
				T maxi() const { return _function::internalMaxi<T>(x, _function::internalMaxi<T>(y, z)); }
			};

			template <typename T> inline Vec3<T>& operator ++ (Vec3<T>& v) { ++v; return v; } 
			template <typename T> inline Vec3<T>& operator -- (Vec3<T>& v) { --v; return v; } 
			template <typename T> inline Vec3<T> operator ++ (Vec3<T>& v, int) { Vec3<T> a = v; ++a; return a; } 
			template <typename T> inline Vec3<T> operator -- (Vec3<T>& v, int) { Vec3<T> a = v; --a; return a; } 
			...
			template <typename T> inline bool operator == (Vec3<T> v, Vec3<T> f) { return f.x == v.x && f.y == v.y && f.z == v.z; }
			template <typename T> inline Vec3<T> mini(Vec3<T> a, Vec3<T> b) { return Vec3<T>(_function::mini<T>(a.x, b.x), _function::mini<T>(a.y, b.y), _function::mini<T>(a.z, b.z)); }
			template <typename T> inline Vec3<T> maxi(Vec3<T> a, Vec3<T> b) { return Vec3<T>(_function::maxi<T>(a.x, b.x), _function::maxi<T>(a.y, b.y), _function::maxi<T>(a.z, b.z)); }
			template <typename T> inline Vec3<T> floor(Vec3<T> a) { return Vec3<T>(_function::floor<T>(a.x), _function::floor<T>(a.y), _function::floor<T>(a.z)); }
			template <typename T> inline Vec3<T> ceil(Vec3<T> a) { return Vec3<T>(_function::ceil<T>(a.x), _function::ceil<T>(a.y), _function::ceil<T>(a.z)); }
			template <typename T> inline Vec2<T> abs(Vec2<T> a) { return Vec2<T>(_function::abs<T>(a.x), _function::abs<T>(a.y)); }
			template <typename T> inline Vec3<T> abs(Vec3<T> a) { return Vec3<T>(_function::abs<T>(a.x), _function::abs<T>(a.y), _function::abs<T>(a.z)); }
